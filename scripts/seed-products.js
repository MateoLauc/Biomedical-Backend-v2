...
function parseProductMarkdown(content) {
    ...
    let currentMainCategory = null; // Track which main category we're in
    let currentCategory = null;
    let currentProduct = null;
    let currentSection = null;
    let categoryDescriptionLines = [];
    for (let i = 0; i < lines.length; i++) {
        ...
        const isMainCategory = mainCategories.some((cat) => line.trim().toUpperCase() === cat.toUpperCase());
        // Check if this is a sub-category header
        const isSubCategory = Object.keys(subCategoryMap).some((subCat) => line.trim().toLowerCase() === subCat.toLowerCase() ||
        ...
        if (isSubCategory) {
            ...
            const subCatName = Object.keys(subCategoryMap).find((subCat) => line.trim().toLowerCase() === subCat.toLowerCase() ||
                line.trim().toUpperCase() === subCat.toUpperCase()) || line;
        ...
        const isProductName = currentSection !== "indication" && // Don't treat indication lines as products
