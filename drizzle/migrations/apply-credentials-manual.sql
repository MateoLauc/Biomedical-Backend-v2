-- Run this in Neon SQL Editor (or any PostgreSQL client) to create credentials tables.
-- Safe to run multiple times (uses IF NOT EXISTS / IF NOT EXISTS for column).

-- 1. Enum for submission status
DO $$ BEGIN
...
-- 2. Submissions table (one per user)
...
-- 3. Documents table (many per submission)
...
-- 4. Form data column (Part A/B/C/D JSON)
